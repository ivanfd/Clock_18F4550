...
uint8_t day_mess = 0;
uint8_t dst_flag = 0;
uint8_t const compile_date[12] = "Feb 04 2025";
uint8_t const compile_time[9] = "16:38:04";
uint8_t valuesAM[5];
uint8_t ErrAM;
uint16_t Rh, Td;
...
asm("\tpsect eeprom_data,class=EEDATA,noexec"); asm("\tdb\t" "4" "," "2" "," "1" "," "2" "," "1" "," "1" "," "1" "," "1");

# 55
asm("\tpsect eeprom_data,class=EEDATA,noexec"); asm("\tdb\t" "1" "," "2" "," "1" "," "0" "," "0" "," "0" "," "0" "," "0");
...
# 90
void home_temp(void) {
uint8_t fptmp, i, tmp1, tmp2, tmp3;
uint16_t t_temp;
...
if ((type_temp == 1) || (temperature == 0)) {
fptmp = temperature % 10;
t_temp = temperature / 10;
if (fptmp >= 6) t_temp += 1;

if (!(t_temp / 10))
{
pic_to_led(3, 1, &Dis_Buff);
putchar_down(13, t_temp, pFont);
...
if ((type_temp == 1) || (temperature_radio == 0)) {
fptmp = temperature_radio % 10;
t_temp = temperature_radio / 10;
if (fptmp >= 6) t_temp += 1;
...
text_buf[0] = ((temperature_radio / 100) % 10) + 48;
text_buf[1] = ((temperature_radio / 10) % 10) + 48;
text_buf[3] = (temperature_radio % 10) + 48;
text_buf[2] = '.';
text_buf[4] = '°';
text_buf[5] = 0;
if (text_buf[0] != 48) {
...
EUSART_Write('_');
EUSART_Write(((temperature / 100) % 10) + 48);
EUSART_Write(((temperature / 10) % 10) + 48);
EUSART_Write((temperature % 10) + 48);
EUSART_Write('_');
break;
case 'd':

EUSART_Write(((Td / 100) % 10) + 48);
...
strcpy(text_buf, "Clock v3.0.1 by Ivan_fd ");
strcat(text_buf, compile_date);
strcat(text_buf, " ");
strcat(text_buf, compile_time);
